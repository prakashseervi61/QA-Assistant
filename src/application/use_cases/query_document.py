...
class QueryDocumentUseCase:
    ...
    async def execute(
        ...
        try:
            ...
            logger.info("Querying documents (conversation=%s, top_k=%d)", resolved_id, top_k)

            user_message = Message(role="user", content=question)
            await self._conversation_repo.add_message(conversation.id, user_message)

            try:
                rag_result = await self._rag_engine.query(question=question, top_k=top_k)
            except Exception as exc:
                logger.error("RAG engine failed for %s: %s", resolved_id, exc, exc_info=True)
                raise QueryDocumentError(f"RAG engine failed to process the query: {exc}") from exc

            answer = rag_result.get("answer", "")
            sources = rag_result.get("sources", [])
            confidence = rag_result.get("confidence", 0.0)

            assistant_message = Message(role="assistant", content=answer, sources=sources)
            await self._conversation_repo.add_message(conversation.id, assistant_message)
    ...
    async def execute_stream(
        ...
        try:
            conversation = await self._resolve_conversation(conversation_id)
            resolved_id = str(conversation.id)
            logger.info("Streaming query (conversation=%s, top_k=%d)", resolved_id, top_k)
            ...
            try:
                async for chunk in self._rag_engine.query_stream(question=question, top_k=top_k):
                    full_answer_parts.append(chunk)
                    yield {"type": "chunk", "content": chunk}
            except Exception as exc:
                logger.error("RAG stream failed for %s: %s", resolved_id, exc, exc_info=True)
            ...
            try:
                ...
                logger.warning("Failed to fetch sources after stream for %s: %s", resolved_id, exc)
                rag_result = {}

            sources = rag_result.get("sources", [])
            confidence = rag_result.get("confidence", 0.0)

            assistant_message = Message(role="assistant", content=full_answer, sources=sources)
            await self._conversation_repo.add_message(conversation.id, assistant_message)
            await self._conversation_repo.save_conversation(conversation)
            ...
            logger.error("Unexpected error in query_document stream: %s", exc, exc_info=True)
            raise QueryDocumentError(f"Failed to stream query documents: {exc}") from exc
